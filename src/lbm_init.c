...
void init_cond_velocity_0_density_1(Mesh* mesh)
{
    ...
    // Loop on all cells
    for (uint32_t i = 0; i < mesh->width; i++) {
        for (uint32_t j = 0; j < mesh->height; j++) {
            for (uint32_t k = 0; k < DIRECTIONS; k++) {
...
{
    // Loop on nodes
    for (uint32_t i = mesh_comm->x; i < mesh->width + mesh_comm->x; i++) {
        for (uint32_t j = mesh_comm->y; j < mesh->height + mesh_comm->y; j++) {
            if (((i - OBSTACLE_X) * (i - OBSTACLE_X)) +
                    ((j - OBSTACLE_Y) * (j - OBSTACLE_Y)) <=
                OBSTACLE_R * OBSTACLE_R)
            {
                *(lbm_cell_type_t_get_cell(mesh_type, i - mesh_comm->x,
                                           j - mesh_comm->y)) =
                    CELL_BOUNCE_BACK;
                             ...
                             lbm_comm_t const* mesh_comm)
{
    ...
    if (mesh_comm->left_id == -1) {
        for (uint32_t j = 1; j < mesh->height - 1; j++) {
    ...
    if (mesh_comm->right_id == -1) {
        for (uint32_t j = 1; j < mesh->height - 1; j++) {
            *(lbm_cell_type_t_get_cell(mesh_type, mesh->width - 1, j)) =
                CELL_RIGHT_OUT;
        }
    }
        ...
        for (uint32_t i = 0; i < mesh->width; i++) {
            for (uint32_t k = 0; k < DIRECTIONS; k++) {
                ...
                Mesh_get_cell(mesh, i, mesh->height - 1)[k] =
                    compute_equilibrium_profile(v, rho, k);
                // Mark as bounce back
                *(lbm_cell_type_t_get_cell(mesh_type, i, mesh->height - 1)) =
                    CELL_BOUNCE_BACK;
            }
        }
    }

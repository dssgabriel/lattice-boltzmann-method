 ...
 **/
void lbm_comm_sync_ghosts_horizontal(lbm_comm_t* mesh, Mesh* mesh_to_process,
    ...
    switch (comm_type) {
        case COMM_SEND:
            for (size_t y = 0; y < mesh->height - 2; y++) {
                MPI_Send(&Mesh_get_col(mesh_to_process, x)[y], DIRECTIONS,
                         MPI_DOUBLE, target_rank, 0, MPI_COMM_WORLD);
            }
            break;
        case COMM_RECV:
            for (size_t y = 0; y < mesh->height - 2; y++) {
                MPI_Recv(&Mesh_get_col(mesh_to_process, x)[y], DIRECTIONS,
 ...
 * @param y Y coordinate to use.
 **/
void lbm_comm_sync_ghosts_vertical(Mesh* mesh_to_process,
                                   lbm_comm_type_t comm_type, int target_rank,
                                   uint32_t y)
{
    // if target is -1, no comm
        ...
        case COMM_SEND:
            for (size_t x = 1; x < mesh_to_process->width - 2; x++) {
                for (size_t k = 0; k < DIRECTIONS; k++) {
                    MPI_Send(&Mesh_get_cell(mesh_to_process, x, y)[k], 1,
            ...
            for (size_t x = 1; x < mesh_to_process->width - 2; x++) {
                for (size_t k = 0; k < DIRECTIONS; k++) {
                    MPI_Recv(&Mesh_get_cell(mesh_to_process, x, y)[k],
                             DIRECTIONS, MPI_DOUBLE, target_rank, k,
...
void lbm_comm_ghost_exchange(lbm_comm_t* mesh, Mesh* mesh_to_process)
{
    ...
    // Left to right phase
    lbm_comm_sync_ghosts_horizontal(mesh, mesh_to_process, COMM_SEND,
                                    mesh->right_id, mesh->width - 2);
    lbm_comm_sync_ghosts_horizontal(mesh, mesh_to_process, COMM_RECV,
                                    mesh->left_id, 0);
    ...
    lbm_comm_sync_ghosts_horizontal(mesh, mesh_to_process, COMM_SEND,
                                    mesh->left_id, 1);
    lbm_comm_sync_ghosts_horizontal(mesh, mesh_to_process, COMM_RECV,
                                    mesh->right_id, mesh->width - 1);
    // Prevend comm mixing to avoid bugs
    MPI_Barrier(MPI_COMM_WORLD);

    // Top to bottom phase
    lbm_comm_sync_ghosts_vertical(mesh_to_process, COMM_SEND,
                                  mesh->bottom_id, mesh->height - 2);
    lbm_comm_sync_ghosts_vertical(mesh_to_process, COMM_RECV,
                                  mesh->top_id, 0);
    // Prevend comm mixing to avoid bugs
    MPI_Barrier(MPI_COMM_WORLD);

                   ...
                   uint32_t width, uint32_t height)
{
    ...
    // int nb_x = lbm_helper_pgcd(comm_size, width);
                                     ...
                                     uint32_t x)
{
    ...
    switch (comm_type) {
        case COMM_SEND:
            MPI_Send(Mesh_get_col(mesh_to_process, x), DIRECTIONS, MPI_DOUBLE,
                                   ...
                                   uint32_t y)
{
    ...
    switch (comm_type) {
        case COMM_SEND:
            ...
            MPI_Send(mesh->buffer, DIRECTIONS * (mesh_to_process ->width - 2), MPI_DOUBLE, target_rank, 0,
                     MPI_COMM_WORLD);
            break;
        case COMM_RECV:

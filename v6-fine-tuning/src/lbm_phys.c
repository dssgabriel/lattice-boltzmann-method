...
/// mesh.
static
double const direction_a[DIRECTIONS] = {
    0.0, 1.0, 0.0, -1.0, 0.0, 1.0, -1.0, -1.0, 1.0
};
static
double const direction_b[DIRECTIONS] = {
...
void get_cell_velocity(Vector v, lbm_mesh_cell_t const cell,
                       double cell_density)
{
    double inv_cell_density = 1.0 / cell_density;
    ...
    v[0] = _mm512_reduce_add_pd(_mm512_mul_pd(vcell, dir_a)) * inv_cell_density;
    v[1] = _mm512_reduce_add_pd(_mm512_mul_pd(vcell, dir_b)) * inv_cell_density;
}
                                   ...
                                   int direction)
{
    ...
    // Compute `e_i * v_i / c`
    double const p1 = direction_a[direction] * velocity[0] + direction_b[direction] * velocity[1];
                                            ...
                                            lbm_mesh_cell_t cell, size_t id_y)
{
    ...
    double const rho =
        (cell[0] + cell[2] + cell[4] + 2 * (cell[3] + cell[6] + cell[7])) / (1.0 - v);
    ...
    for (size_t j = 0; j < mesh_out->height; j++) {
        for (size_t i = 0; i < mesh_out->width; i++) {
            ...
            for (size_t k = 0; k < DIRECTIONS; k++) {
                ...
                ssize_t ii = (i + direction_a[k]);
                ssize_t jj = (j + direction_b[k]);
